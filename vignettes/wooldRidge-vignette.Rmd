...
Calculated $\hat{\mu^2_t}$ by squaring the residuals contained in the `return_AR` model object and store the results in the variable named `return_mu2`. Then regress the `return_1` data also contained in the `return_AR` objects model matrix. Note the `data` arguement in the call to `lm` below.

```{r}
return_mu2 <- return_AR$residuals^2

mu_squared_hat <- lm(return_mu2 ~ return_1, data = return_AR$model)

summary(mu_squared_hat)
```
