...
```{r, tidy=TRUE}
par(mfrow=c(1,3))
plot(y = wage1$lwage, x = wage1$educ, xaxt="n", frame = FALSE, main = "Wages vs. Education", xlab = "years of education", ylab = "Hourly wages")
axis(side = 1, at = c(0,6,12,18))
abline(lm(lwage ~ educ, data=wage1), col = "blue", lwd=2)

plot(y = wage1$lwage, x = wage1$exper, xaxt="n", frame = FALSE, main = "Wages vs. Experience", xlab = "years of experience", ylab = "Hourly wages")
axis(side = 1, at = c(0,12.5,25.50,38,52))
abline(lm(lwage ~ exper, data=wage1), col = "blue", lwd=2)

plot(y = wage1$lwage, x = wage1$tenure, xaxt="n", frame = FALSE, main = "Wages vs. Tenure", xlab = "years with emloyer", ylab = "Hourly wages")
...
First, use the `subset` function and create a new data object which
contains those observations which occurred in `1987` and are not `union`.

```{r}
jtrain_1987_non <- subset(jtrain, subset = (year == 1987 & union == 0))
```

Now, explore relationships among the independent and dependent variables visually.

```{r, tidy=TRUE}
par(mfrow=c(1,3))
plot(y = jtrain_1987_non$lscrap, x = jtrain_1987_non$hrsemp, frame = FALSE, xlab = "Total (hours/employees) trained", ylab = "Log(scrap rate per 100)")
#axis(side = 1, at = c(0,6,12,18))
abline(lm(lscrap ~ hrsemp, data=jtrain_1987_non), col = "blue", lwd=2)

plot(y = jtrain_1987_non$lscrap, x = jtrain_1987_non$lsales, frame = FALSE, xlab = "Log(annual sales $)", ylab = "Log(scrap rate per 100)")
#axis(side = 1, at = c(0,12.5,25.50,38,52))
abline(lm(lscrap ~ lsales, data=jtrain_1987_non), col = "blue", lwd=2)

plot(y = jtrain_1987_non$lscrap, x = jtrain_1987_non$lemploy, frame = FALSE, xlab = "Log(# employees at plant)", ylab = "Log(scrap rate per 100)")
#axis(side = 1, at = c(0,11,22,33,44))
abline(lm(lscrap ~ lemploy, data=jtrain_1987_non), col = "blue", lwd=2)
```
...
```{r, tidy=TRUE}
linear_model <- lm(lscrap ~ hrsemp + lsales + lemploy, data = jtrain_1987_non)
```
...
Plot the coefficients, representing the impact of each variable on $log($`scrape`$)$ for a quick comparison.
