...
library(xts) # load eXtensible Time Series pkg for time series

intdef3 <- xts(intdef[ ,-1], order.by = as.Date(intdef$year)) #create xts object
intdef3 <- intdef3[ ,c("i3", "inf", "def")] # extract variables
colnames(intdef3) <- c("Tbill3mo", "cpi", "deficit") # give clear names

plot(intdef3, main = "Inflation, Deficits, and Interest Rates",
     legend.loc = "topleft") # plot.xts to explore

tbill_model <- lm(Tbill3mo ~ cpi + deficit, data = intdef3) # model relationship
```
