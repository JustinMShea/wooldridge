...
index <- as.yearmon(intdef$year + 11/12)
...
# Inflation
inflation_url <- "https://fred.stlouisfed.org/data/FPCPITOTLZGUSA.txt"
# use the read.zoo function to read in the file
inflation <- read.zoo(file = inflation_url,
                      skip = 17, 
                      index.column = 1, 
                      header = TRUE,
                      format = "%Y-%m-%d", 
                      FUN = as.yearmon)
# read.zoo creates an index too. Lets move Jan to Dec
index(inflation) <- index(inflation) +11/12 
# transform to xts time series object
inflation.xts <- as.xts(inflation)
# update column names
colnames(inflation.xts) <- "inflation"

## Deficit, percent of GDP: Federal outlays - federal receipts
 # Download outlays
outlays_url <-"https://fred.stlouisfed.org/data/FYFRGDA188S.txt"
outlays <- read.zoo(file = outlays_url,
                    skip = 17, 
                    index.column = 1, 
                    header = TRUE, 
                    format = "%Y-%m-%d", 
                    FUN = as.yearmon)
# read.zoo creates an index too. Lets move Jan to Dec
index(outlays) <- index(outlays) + 11/12
...
# Download receipts
receipts_url <- "https://fred.stlouisfed.org/data/FYONGDA188S.txt"
receipts <- read.zoo(file = receipts_url,
                     skip = 17, 
                     index.column = 1, 
                     header = TRUE, 
                     format = "%Y-%m-%d", 
                     FUN = as.yearmon)
# read.zoo creates an index too. Lets move Jan to Dec
index(receipts) <- index(receipts) + 11/12
# transform to xts object
...
# Merge
intdef_updated <- merge(Tbill_3mo.xts, 
                        inflation.xts, 
                        deficit.xts)

# remove leading and trailing NAs with na.trim 
