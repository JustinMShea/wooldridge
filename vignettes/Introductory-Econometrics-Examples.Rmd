...
This vignette contains examples from every chapter of _Introductory Econometrics: A Modern Approach, 6e_ by Jeffrey M. Wooldridge. Each example illustrates how to load data, build econometric models, and compute estimates with `r icon::fa_r()`.

In addition, the **Appendix** cites good sources on using `r icon::fa_r()` for econometrics. 
...
```{r, tidy=TRUE}
#par(mfrow=c(1,3))
plot(y = wage1$lwage, x = wage1$educ, col = "blue", xaxt="n", frame = FALSE, main = "Wages vs. Education", xlab = "years of education", ylab = "Hourly wages")
axis(side = 1, at = c(0,6,12,18))

plot(y = wage1$lwage, x = wage1$exper, col = "blue", xaxt="n", frame = FALSE, main = "Wages vs. Experience", xlab = "years of experience", ylab = "Hourly wages")
axis(side = 1, at = c(0,12.5,25.50,38,52))

plot(y = wage1$lwage, x = wage1$tenure, col = "blue", xaxt="n", frame = FALSE, main = "Wages vs. Tenure", xlab = "years with emloyer", ylab = "Hourly wages")
...
Plot the coefficients for a quick comparison.

```{r}
barplot(hourly_wage_model$coefficients[-1])
```
...
This time, add the `subset` argument of the linear model function to indicate that we only want to estimate the model using observations which occur in `1987` and are not `union`.
...
```{r, tidy=TRUE}
linear_model <- lm(lscrap ~ hrsemp + lsales + lemploy, data = jtrain, 
...
```{r}
barplot(linear_model$coefficients[-1])
