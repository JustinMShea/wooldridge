...
kable(data.frame(Avg.Rooms, Rooms.linear, Rooms.quadratic, row.names=row_names), 
      caption="Percent Change per variable for each increase in Average Rooms", 
      row.names = TRUE, digits=2)
```


Observe the difference visually by graphing the log(selling price) against the number of rooms. Superimpose the linear model and again for the quadratic model and examine the difference.
The blue vertical line represents the quadratic model turning point of 4.38 rooms.

```{r, tidy=TRUE}
par(mfrow=c(1,2))

plot(y = hprice2$lprice, x = hprice2$rooms, xaxt="n", pch = 21, bg = "lightgrey",
     main = "lprice ~ rooms", xlab = "Rooms", ylab = "")
 mtext(side=2, line=2, "Log( selling price )", cex=1.25)
 axis(side = 1, at = c(3, round(turning_point,2), 5.5, 7, max(hprice2$rooms)))
 abline(lm(lprice ~ rooms, data = hprice2), col="red", lwd=2.5)
 abline(v=turning_point,  col="blue")

visreg(housing_model_6.2, "rooms", band=FALSE, line=list(col="red"),
       points=list(cex=1, pch = 21, col="black", bg = "lightgrey"),
       main = "lprice ~ rooms + I(rooms^2)", xlab = "Rooms", ylab="",
       xaxt="n")
 abline(v=turning_point,  col="blue")
 axis(side = 1, at = c(3, round(turning_point,2), 5.5, 7, max(hprice2$rooms)))
```
