...
```{r, tidy=TRUE}
plot(y = wage1$wage, x = jitter(wage1$educ), col = "darkgreen", pch = 21, bg = "lightgrey",     
     cex=1, xaxt="n", frame = FALSE, main = "Wages vs. Education, 1976", 
...
```{r, tidy=TRUE}
plot(y = wage1$lwage, x = jitter(wage1$educ), main = "A Log Wage Equation", 
     col = "darkgreen", pch = 21, bg = "lightgrey", cex=1,
...
```{r, tidy=TRUE, fig.height=8, eval=FALSE, echo=FALSE}
par(mfrow=c(2,2))
...
```{r, tidy = TRUE}
housing_standardized <- lm(scale(price) ~ 0 + scale(nox) + scale(crime) + 
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, tidy=TRUE}
stargazer(type = "html", housing_model_4.5, housing_model_6.2, single.row = TRUE, 
          header = FALSE, digits=5, column.labels = c("Example 4.5", "Example 6.2"))
```
...
Observe the percent change at various rooms for both the linear model from example 4.5 as well as the quadratic model in example 6.2. 

```{r, tidy=TRUE, fig.show="hold"}
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, tidy=TRUE}
stargazer(type = "html",econ_crime_model, econ_crim_poisson,  single.row = TRUE, header = FALSE, digits=5)
```
