...
Fortunately, `R` includes this in the model `lm` diagnostics:
...
Plot the $t$ statistics for a visual comparison of coefficients:

```{r}
barplot(summary(hourly_wage_model)$coefficients[-1, "t value"], ylab = "t statistic", 
        main = "t statistics of Hourly Wage Equation")
```
...
```{r, echo=FALSE}
plot(y = hprice3$price, x = hprice3$dist, main = "Housing Prices and Distance From an Incinerator", xlab = "Distance to Incinerator in feet", ylab = "Selling Price",  frame = FALSE, pch = 21, bg = "lightgrey")
...
Adding `area` improves the quality of the model, but also substantially reduces the
coefficient for `dist`. Plotting illustrates the a larger coefficient for `area`.
...
mtext(side=2, line=2, "Log(price)", cex=1.25)
...
From  J. Grogger (1991), _Certainty vs. Severity of Punishment_, Economic Inquiry 
29, 297-309. Professor Grogger kindly provided a subset of the data he used in his 
article.
...
```{r, tidy = TRUE}
crime_model <- lm(narr86 ~ pcnv + ptime86 + qemp86, data = crime1)
```

Create a new variable `crime_u` containing the residuals $\tilde{\mu}$ from the above regression.

```{r}
crime_u <- crime_model$residuals
```

Next, regress `pcnv, ptime86, qemp86, avgsen`, and `tottime`, against the residuals 
$\tilde{\mu}$ saved in `crime_u`.

$$\tilde{\mu} = \beta_1pcnv + \beta_2avgsen + \beta_3tottime + \beta_4ptime86 + \beta_5qemp86$$


```{r, tidy = TRUE}
restricted_model <- lm(crime_u ~ pcnv + ptime86 + qemp86 + avgsen + tottime, data = crime1)

```

```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, tidy=TRUE}
stargazer(type = "html", crime_model, restricted_model,  single.row = TRUE, header = FALSE, digits=5)
```
...
Estimate the coefficient with the usual `lm` regression model.
...
Modify the housing model from **`example 6.1`**, adding a quadratic term in _rooms_: 
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, tidy=TRUE}
stargazer(type = "html", housing_standardized, housing_interactive, single.row = TRUE, header = FALSE, digits=5)
```

Estimate the minimum turning point of the `rooms` interactive term:

$$x = \frac{\hat{\beta_1}}{2\hat{\beta_2}}$$

```{r, tidy=TRUE}
beta_1 <- summary(housing_interactive)$coefficients["rooms",1] 
beta_2 <- summary(housing_interactive)$coefficients["I(rooms^2)",1]
...
```{r, tidy=TRUE}
Rooms <- c(min(hprice2$rooms), 4, turning_point, 5, 5.5, 6.5, 7.5, max(hprice2$rooms))
...
quadratic <- abs(100*summary(housing_interactive)$coefficients["rooms",1] + 200*summary(housing_interactive)$coefficients["I(rooms^2)",1]*rooms)

housing_model_frame <- model.frame(housing_interactive)
...
linear model as well as a quadratic model and examine the difference.

```{r, tidy=TRUE}
rooms_interaction <- lm(lprice ~ rooms + I(rooms^2), data = hprice2)

par(mfrow=c(1,1))
plot(y = hprice2$lprice, x = hprice2$rooms, xaxt="n", frame = FALSE, main = "Effects of Pollution on Housing Prices", xlab = "Rooms", ylab = "Log(selling price)")
axis(side = 1, at = c(min(hprice2$rooms), 4, 5, 6, 7, 8, max(hprice2$rooms)))
abline(lm(lprice ~ rooms, data = hprice2), col="blue", lwd=2.5)
points(hprice2$rooms, fitted(rooms_interaction), col = "red", pch=20)
