...
```{r}
plot(y = wage1$lwage+1, x = wage1$educ, col = "darkgreen", main = "Wages vs. Education, 1976", xlab = "years of education", ylab = "log (hourly wages)")
```
...
View the plot again, with the linear model.
```{r}
plot(y = wage1$lwage, x = wage1$educ, main = "A Log Wage Equation", xlab = "years of education", ylab = "log of average hourly wages", frame = FALSE)
...
linear_model <- lm(lscrap ~ hrsemp + lsales + lemploy, data = jtrain, 
                   subset = (jtrain$year == 1987 & jtrain$union == 0))
```
