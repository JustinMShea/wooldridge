...
kable(data.frame(Avg.Rooms, Rooms.linear, Rooms.quadratic, row.names = c("Minimum", "4 Rooms", "turning point", "5.5 rooms", "equal effect", "7.5 rooms", "Maximum")), row.names = TRUE, digits=2,
      caption="Percent Change per variable for each increase in Average Rooms")

```


Observe the difference visually by graphing the log(selling price) against the number of rooms. Superimpose a the linear model and then do the same for the quadratic model to examine the difference.
...
visreg(housing_model_6.2, "rooms", band=FALSE, line=list(col="red"),
    ...
    xaxt="n", main = "lprice ~ rooms + I(rooms^2)", xlab = "Rooms", ylab="")
abline(v=turning_point,  col="blue")
axis(side = 1, at = c(3, round(turning_point,2), 5.5, 7, max(hprice2$rooms)))
```
