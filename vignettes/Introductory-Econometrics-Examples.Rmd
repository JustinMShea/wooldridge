...
visreg(hourly_wage_model, "educ", band=FALSE, jitter = TRUE, points=list(col="darkgreen", cex=1.1, pch=1), frame=FALSE, xaxt="n", main = "years of education", xlab = "", ylab = "")
mtext(side=2, line=2.5, "Hourly wages", cex=1.25)
axis(side = 1, at = c(0,6,12,18))

visreg(hourly_wage_model, "exper", band=FALSE, jitter = TRUE, points=list(col="darkgreen", cex=1.1, pch=1), frame=FALSE, xaxt="n", main = "years of experience", xlab = "", ylab = "")
axis(side = 1, at = c(0,12.5,25,37.5,50))

visreg(hourly_wage_model, "tenure", band=FALSE, jitter = TRUE, xaxt="n", points=list(col="darkgreen", cex=1.1, pch=1), frame=FALSE,  main = "years of employer", xlab = "", ylab = "")
axis(side = 1, at = c(0,11,22,33,44))
```
...
Compare results, and observe that while the coefficients are much easier to compare and contrast with the new model, the **$R^2$** statistic remains identical.
...
Avg.Rooms <- c(min(hprice2$rooms), 4, turning_point, 5.5, 6.4216564, 7.5, max(hprice2$rooms))
Rooms.quadratic <-  100*(beta_1 + 2*beta_2*Avg.Rooms)
Rooms.linear <- 100*rep(summary(housing_model_4.5)$coefficients["rooms",1], length(Avg.Rooms))

row_names <- c("Minimum", "4 Rooms", "turning point", "5.5 rooms", "equal effect", "7.5 rooms", "Maximum")
rooms_data <- data.frame(Avg.Rooms, Rooms.linear, Rooms.quadratic, row.names=row_names)
kable(rooms_data, caption="Rate of Change in Selling Price per Room by Model", row.names=TRUE, digits=2)

plot(x=rooms_data$Avg.Rooms, y=rooms_data$Rooms.quadratic, type="l", lwd=2, col="red",
     frame=FALSE, ylab="Percent", xlab="Rooms", main="Rate of Change in Selling Price per Room by Model")
abline(h=rooms_data$Rooms.linear, col="blue", lwd=2)
abline(h=0, col="green", lty=2)
abline(v=6.4216564, lty=2)
legend(3.56, 50, legend=c("Example 6.2: Quadratic", "Example 4.5: Linear", 
                          "Quadratic Turning Point", "Coefficient Equality"), 
                           col=c("red", "blue", "green", "black"), 
       lty=c(1,1,2,2), lwd=c(2,2,1,1), cex=0.8, box.lty=0,text.font=2)
```



Observe the difference by graphing the log(selling price) against the number of rooms, 
superimposing the linear and quadratic models for comparison. The blue 
vertical line represents the quadratic model turning point of 4.38 rooms.
...
plot(y = jitter(hprice2$lprice), x = jitter(hprice2$rooms), xaxt= "n" , frame = FALSE, 
     pch = 21, bg = "lightgrey", main = "lprice ~ rooms", xlab = "Rooms", ylab = "")
  mtext(side=2, line=2, "Log( selling price )", cex=1.25)
  axis(side = 1, at = c(round(turning_point,2), 5.5, 7, max(hprice2$rooms)))
  abline(lm(lprice ~ rooms, data = hprice2), col = "red", lwd = 2.5)
  abline(v=turning_point,  col="blue", lty = 2)
  legend("bottomright", legend="Model", col="red", lty=1, lwd=2 , cex=0.8, box.lty=0,text.font=2)

visreg::visreg(housing_model_6.2, "rooms", band=FALSE, axes=FALSE, xlab="Rooms", ylab="", line=list(col="red"), 
               points=list(cex=1, pch=21, col="black", bg="lightgrey"), main="lprice ~ rooms + I(rooms^2)")
  abline(v = turning_point,  col = "blue", lty = 2)
  axis(side = 1, at = c(round(turning_point, 2), 5.5, 7, max(hprice2$rooms)))
    legend("bottomright", legend="Turning", col="blue", lty=2, lwd=1, cex=0.8, box.lty=0,text.font=2)
```
...
Estimate a new model, `barium_seasonal` which accounts for seasonality by adding dummy variables contained in the data. Although it adds a few key strokes, sometimes storing the model syntax in it's own object prior to passing it to the estimation function reads more clearly. Observe this personal preference below.

```{r}
formula <- lchnimp ~ lchempi + lgas + lrtwex + befile6 + affile6 + afdec6 + 
                     feb + mar + apr + may + jun + jul + aug + sep + oct + nov + dec

barium_seasonal <- lm(formula, data = barium)
```
...
```{r}
library(prais)
barium_prais_winsten <- prais_winsten(lchnimp ~ lchempi + lgas + lrtwex + befile6 + affile6 + afdec6, 
                                      data = barium)
```
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
stargazer(type = "html",wage_educ_model, fatheduc_model, wage_educ_IV, 
...
```{r}
data("mroz")
wage_educ_exper_IV <- ivreg(lwage ~ educ + exper + expersq | exper + expersq + motheduc + fatheduc, 
                            data = mroz)
```
